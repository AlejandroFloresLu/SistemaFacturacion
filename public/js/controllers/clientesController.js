 ...
 * CRUD real conectado a la API REST via ClienteModel.
 */
document.addEventListener('DOMContentLoaded', function () {
    ...
    let cliFiltrados   = [];
    let elimId         = null; // id de BD del cliente a eliminar
    ...
    window.guardarNuevoCliente = async function () {
        ...
        try {
            await ClienteModel.create({ ruc, nombre: nom, apellido: ape, tel, email });
    ...
    window.editarCliente = function (id) {
        const c = DB_CLIENTES.find(x => x.id === id);
        if (!c) return;
        document.getElementById('editCliIdx').value      = id;
    ...
    window.guardarEdicionCliente = async function () {
        const id     = +document.getElementById('editCliIdx').value;
        ...
        try {
            await ClienteModel.update(id, { nombre: nom, apellido: ape, tel, email });
    ...
    // ── Eliminar Cliente ──────────────────────────────────────────────────────
    window.pedirEliminarCliente = function (id, nombre) {
        elimId = id;
        document.getElementById('elimCliNombre').textContent = nombre;
    ...
    async function recargarClientes() {
        ClienteModel.clearCache();
        DB_CLIENTES  = await ClienteModel.getAll();
